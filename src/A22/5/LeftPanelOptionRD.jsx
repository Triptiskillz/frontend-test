...
function LeftPanelOptionRD({ options, allOptions, OnOptionChange }) {
            ...
            {arr.map((opt) => (
                <div className='form-check' key={opt}>
                    <input
                        ...
                        type="radio"
                        value={opt.value}
                        checked={value.find((val) => val == opt.value)||false}
                        onChange={handleChange}
                    />
                    <lable className='form-check-label'>{opt.display}</lable>
                </div>
            ))}
   ...
   let arr =[{display:"Below 10,000",value:"Below"},{display:"10,000 or More",value:"More"}]
    let {price=''} = options;
    ...
    return (
        <div className="row border bg-light">
            ...
            <div className="col-12">{makeCheckboxes(arr,  price.split(','), "price", "Desired Price")}</div>
